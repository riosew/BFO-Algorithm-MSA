...
#crea un ciclo para repetir la corrida entera
    for corridaNum in range(1, 2):   #31
        ...
        numRandomBacteria = 0
        iteraciones = 200
        tumbo = 100                                            #numero de gaps a insertar 
        nado = 1
        secuencias = list()
        #path de salida csv
        ...
        for it in range(iteraciones):
            ...
            if (veryBest[0] == None) or (bestFitness > veryBest[1]): #Remplaza el mejor 
                # print("Remplaza el mejor")
                veryBest[0] = bestIdx
                veryBest[1] = bestFitness
                veryBest[2] = copy.deepcopy(poblacion[bestIdx])
            #operadorBacterial.replaceWorst(poblacion, veryBest[0])
            operadorBacterial.replace3Worst(poblacion,1)
            
            
            elapsed_time = time.time() - start_time    
            corridaNum, dAttr, wAttr, hRep, wRep, numSec, tumbo, nado, numRandomBacteria, numeroDeBacterias, iteraciones = parametros
            print(", dAttr:,", dAttr, ",wAttr:,", wAttr, ",hRep:,", hRep, ",wRep:,", wRep, ",numSec:,", numSec, ",tumbo:,", tumbo, ",nado:,", nado, ",numRandomBacteria:,", numRandomBacteria, ",numeroDeBacterias:,", numeroDeBacterias, ",iteraciones:,", iteraciones, ",Best:,", bestIdx, ",Fitness:,", veryBest[1], ",NFE:,", globalNFE, ",tiempo:,", elapsed_time)
